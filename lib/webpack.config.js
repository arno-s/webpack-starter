...
const constructPlugins = (additionalConfig) => {
  ...
  // inlining critical css using critters causes problems when using
  // styled-components so we disable it by default
  if (additionalConfig.enableCriticalInlining) {
    plugins.push(
      new CrittersPlugin({
