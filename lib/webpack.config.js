...
const createWebpackConfiguration = (options) => {
  ...
  const additionalConfig = getAdditionalConfiguration();
  const defaultConfig = constructDefaultConfiguration(options, additionalConfig);
  const additionalWebpackConfig = getAdditionalWebpackConfiguration();
...
const constructPlugins = (additionalConfig) => {
  // Don't add the HTML Webpack plugin, SSR usually defines it's own HTML
